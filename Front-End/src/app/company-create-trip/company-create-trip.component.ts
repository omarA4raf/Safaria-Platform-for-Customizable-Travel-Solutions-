...
// Create a tourData object
const tourData = {
  title: this.trip.title,
  destinationCountry: this.trip.destinationCountry,
  tourismTypes: this.trip.tourismTypes,
  description: this.trip.description,
  schedules: this.trip.availableDates, // Ensure this matches the backend DTO
  freeCancellationDeadline: this.trip.freeCancellationDeadline,
  currency: this.trip.currency,
};

// Append tourData as a JSON string with explicit Content-Type
const tourDataBlob = new Blob([JSON.stringify(tourData)], {
  type: 'application/json', // Set the Content-Type explicitly
});
formData.append('tourData', tourDataBlob, 'tourData.json'); // Add a filename for clarity
...
this.http.post('http://localhost:8080/api/tours/create', formData).subscribe({
  next: (response) => {
